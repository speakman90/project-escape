...
class JWTService
{
    ...
    public function generate(array $header, array $payload, string $secret, int $validity = 2000): string
    {
        if($validity > 0){
    ...
    {
        //On récupère le header et le getPayload
        $header = $this->getHeader($token);
        $payload = $this->getPayload($token);

        //On régnère un token pour
